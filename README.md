...
3. 视口内及附近的英文段落被替换为句法标注：成分名在上、按成分类型着色的细下划线、中文翻译在下；向下滚动增量分析；
4. 解析期间页面右下角有进度胶囊（「句法解析中 n/m」），完成后自动消失；
5. 点击任意成分 → 懒加载该成分的详细解析；
6. 主按钮即状态机：解析中点击=「暂停」，暂停后点击=「继续学习」，全部完成后点击=「恢复网页原文」；
7. 切换模型：设置页「已保存配置」选中目标配置 →「设为启用」（当前启用项带「（启用中）」标记）。
