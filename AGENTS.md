...
- **tokenization 变化同时升两条提示词版本**:core span 与 detail focus 都依赖 Token ID,所以分词改动必须同时提升 `CORE_PROMPT_VERSION` 与 `DETAIL_PROMPT_VERSION`;当前为 `CORE_SCHEMA_VERSION=3`、core `15`(13 面向页面级覆盖重写双端规则;14 让 repair 错误按句分组并用模型所见原始下标;15 新增句末引用附着与章节编号绑定两条教学规则)、detail `8`(MathML 文本改读可见表示后同步提升)。
