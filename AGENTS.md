...
- **本地可判的语法规则必须进双端 validator**:TS/Kotlin 只同步执行代码实际可判的四条硬门:组件序列相邻且 Token 区间连续的两个 `PREDICATE` 合并;去掉标点后恰好一个 lexical word、role 不是 `CONJUNCTION` 且命中**保守的高把握“必须带宾语”介词白名单**的成分并入管辖短语(常见副词/表语/连词兼类词 `after/before/down/off/over/since/until/around/inside/outside` 不收);`COORDINATE_CLAUSE` 数量恰好为 1 时拒绝;`CONJUNCTION` 的 lexical words 至少含一个 FANBOYS。grammar 判定只由结构可信度把关(每个 component 都有可用 range/role/translation、区间句内、有序不重叠、非纯标点),unknown field、translation too long、sentenceId 等非结构错误不得阻断同轮 grammar 诊断,两类错误必须同次返回。错误英文文案逐字一致并进入 repair prompt。
- **tokenization 变化同时升两条提示词版本**:core span 与 detail focus 都依赖 Token ID,所以分词改动必须同时提升 `CORE_PROMPT_VERSION` 与 `DETAIL_PROMPT_VERSION`;当前为 `CORE_SCHEMA_VERSION=3`、core `6`、detail `5`。
