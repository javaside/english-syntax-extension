...
- **tokenization 变化同时升两条提示词版本**:core span 与 detail focus 都依赖 Token ID,所以分词改动必须同时提升 `CORE_PROMPT_VERSION` 与 `DETAIL_PROMPT_VERSION`;当前为 `CORE_SCHEMA_VERSION=3`、core `16`(13 页面级覆盖重写;14 repair 错误按句分组并用模型所见原始下标;15 句末引用与章节编号;16 把冒号处理收口成有优先级的 `COLON_STRUCTURE_RULE`,并让 caption/field label 分别绑定中心词或作为 `INDEPENDENT_ELEMENT`)、detail `9`(MathML 文本变化升 8;点分标识符整体化再次同步升 9)。
