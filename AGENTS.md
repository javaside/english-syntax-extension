...
- **本地可判的语法规则必须进双端 validator**:TS/Kotlin 只同步执行代码实际可判的十六条硬门:组件序列相邻且 Token 区间连续的两个 `PREDICATE` 合并;去掉标点后恰好一个 lexical word、role 不是 `CONJUNCTION` 且命中**保守的高把握“必须带宾语”介词白名单**的成分并入管辖短语(常见副词/表语/连词兼类词 `after/before/down/off/over/since/until/throughout/around/inside/outside` 不收);`COORDINATE_CLAUSE` 出现即拒绝(≥1 个,该角色已废弃,并列句按同层成分平铺、FANBOYS 单独标 `CONJUNCTION`);`CONJUNCTION` 的 lexical words 至少含一个 FANBOYS;`ADVERBIAL`/`ATTRIBUTE` 以从属连词开头时按保守门拒绝:无条件词表 `{although,whereas,unless,lest,whilst}`，`because` 仅第二实词不是 `of` 时拒，`though` 仅含至少 2 个实词时拒;`PREDICATE` 首个 lexical word 是限定词/主格代词/`that` 时拒绝(动词组不可能以它们开头,命中即主语被吞);`PREDICATE` 非首位 lexical words 含限定词时拒绝(限定词是名词短语左边界,`that` 刻意不在这条里);单个成分覆盖全部非标点 token 且句子实词数 ≥ 4 时拒绝;片段语义角色 `{FRAGMENT_HEAD,INDEPENDENT_ELEMENT,APPOSITIVE}` 在实词数 ≤10 时豁免,超过 10 仍拒绝并要求拆主体与修饰语(`MAX_WHOLE_SENTENCE_FRAGMENT_LEXICAL_TOKENS=10` 是挑战集审阅后的启发式,不是语法定律);短完整句整体误标 `FRAGMENT_HEAD` 仍刻意不靠词表判;`COORDINATE_CLAUSE` 首词是从属连词且整句无 `CONJUNCTION` 时另有一条补充错误(有 `CONJUNCTION` 就放行——`Because A, B, and C` 的第一个并列分句本来就这样开头;旧的「≥2 个并列分句须有连词或分号」判据已被废弃门整体覆盖)。五类从句角色的成分只有 1 个 lexical word 时拒绝(从句至少是引导词 + 谓语);`SUBJECT_CLAUSE` 首词必须命中 15 词闭集 `{that,whether,what,whatever,which,whichever,who,whoever,whom,whomever,whose,how,why,when,where}`，刻意不收 `if/however`;`ATTRIBUTIVE_CLAUSE` 的下一个成分是 `OBJECT` / `PREDICATIVE` 时拒绝(定语从句修饰的名词在从句之前,紧跟的宾语/表语一定是从句自己的;`COMPLEMENT` 刻意豁免,宾补结构 `consider the movie that she directed a masterpiece` 合法;主句谓语与主句状语跟在从句后面也合法);名词性角色 `{SUBJECT,OBJECT,PREDICATIVE,COMPLEMENT,ATTRIBUTE,APPOSITIVE}` 的成分,若 `of` 的**前一个 lexical word 命中高把握名词中心词白名单 `OF_HEAD_NOUNS`(当前只收 `development`)**则拒绝,要求在 `of` 前截止并将后置 `of` 短语单列为 `ATTRIBUTE`(从句、`ADVERBIAL`、以 `of` 开头的合法定语豁免;判据刻意收窄到白名单而非「出现非首位 `of`」——后者会误拒系表/宾补的 `made of steel`、`capable of handling failures` 与专名 `Bank of America`,它们 `of` 前不是名词中心;扩表须逐词确认几乎总带后置定语);非从句角色成分的最后一个 lexical word 命中「几乎不可能在短语内悬垂」的介词表(`of/into/onto/upon/within/among/between/despite/during/toward/towards`)时拒绝;五类从句角色豁免(从句内部 `what dreams are made of` 可合法悬垂),短语角色 `near the frontier of` 仍拒绝;`for`/`with`/`at`/`from`/`to` 刻意不收;`FRAGMENT_HEAD` 至多一个且不得与 `FRAGMENT_FORBIDDEN_ROLES` 显式列举的分句级角色(`SUBJECT`/`PREDICATE`/`OBJECT`/`PREDICATIVE`/`COMPLEMENT`、`SUBJECT_CLAUSE`/`OBJECT_CLAUSE`/`PREDICATIVE_CLAUSE`/`ADVERBIAL_CLAUSE` 四类从句、`COORDINATE_CLAUSE`)混用——唯一例外是完整 `ATTRIBUTIVE_CLAUSE` 可紧跟片段主体(fragment-relative 放行,如 `An API` + `that returns JSON responses`,只移出这一类、其余从句门照旧);两条 exact validator error 文案逐字一致(混用文案显式列出仍禁止的角色,不再说 all clause roles),双端 repair prompt 原样引用。这三条从句硬门补的是「`that` 被单独标成定语从句、从句的谓语与宾语平铺到主句层」这一类——那个划分完全通过前几条。**缺主语本身不判**——祈使句本来就没有主语,文档里 `First, install the CLI.` 这类副词开头的祈使句更常见,按缺主语判会大面积误拒;`then` 是副词,不进从属连词表;validator 也**刻意不用词表判「缺限定谓语」**(词形兼类会大面积误拒,模型漏用 `FRAGMENT_HEAD` 靠 prompt/黄金集/真模型评测约束)。grammar 判定只由结构可信度把关(每个 component 都有可用 range/role/translation、区间句内、有序不重叠、非纯标点),unknown field、translation too long、sentenceId 等非结构错误不得阻断同轮 grammar 诊断,两类错误必须同次返回。错误英文文案逐字一致并进入 repair prompt。**最终 component 的 translation 还有双端逐字一致的译文质量门(非语法字段校验)**:必须至少含一个 Unicode Han 字符(TS `\p{Script=Han}` / JVM `\p{IsHan}`,15 个共享边界 case 由 `shared-fixtures/translation-quality.json` 双端 replay),且不得在 NFKC + 大小写折叠 + 空白折叠后等于按 `leadingWhitespace + text` 重建的英文 span——回显与只有大小写/空白差异的回显都拒绝;它与 grammar 错误同轮返回但不阻断 grammar 诊断,进现有至多两轮 repair 配额(不加第三轮),最终失败不写缓存;专名不是省略中文的理由——「保留英文 + 补中文类型」(`JSON 数据格式`/`Spring AI 框架`/`GWTC-5.0 引力波事件目录`/`哈勃常数 H0`)合法,纯 `JSON`/`Spring AI` 回显拒绝;只有纯标点/引用数字与含无歧义数学标记或公式形态的 span 豁免 Han 门,ASCII `/~^_|\\` 不算数学标记(TCP/IP、km/h、A/B testing 仍须中文);detail 的 translation 不强制(渐进增强),renderer 的 `isEchoTranslation()` 保留为流式与旧缓存显示兜底。**黄金集必须整份通过这套校验**(`core-gold-annotations.test.ts` 钉住):新硬门把正确答案判非法比漏判更糟,那会把合法分析送进无意义的修复轮。反过来,**过了 validator 不等于标注对**:2026-08-30 那批 32 句自动生成的标注只跑结构校验就并进了黄金集,9-02 复核出 15 处错标(`that` 单独当从句、`developers` 当主语从句、介词悬空、系表两种口径混用),错标注会让评分器奖励线上正在犯的错。新增黄金句必须人工核语言学正确性,并同时补一条能机器判定的口径断言;黄金集标点总体不覆盖,`fragment-portable-api` 的 `ATTRIBUTE 6..14`(含终止标点)是钉死的唯一例外,不得推广。
...
- **tokenization 变化同时升两条提示词版本**:core span 与 detail focus 都依赖 Token ID,所以分词改动必须同时提升 `CORE_PROMPT_VERSION` 与 `DETAIL_PROMPT_VERSION`;当前为 `CORE_SCHEMA_VERSION=3`、core `17`(13 页面级覆盖重写;14 repair 错误按句分组并用模型所见原始下标;15 句末引用与章节编号;16 把冒号处理收口成有优先级的 `COLON_STRUCTURE_RULE` 并整体化点分标识符;17 钉住名词中心与后置 `of` 短语的自然局部译文)、detail `10`(MathML 文本变化升 8;点分标识符整体化升 9;同一局部译文教学升 10)。
- **准确性改动用黄金集评分,CI 不联网**:`core-gold-annotations.json` + 纯评分器比较整句 exact、span/labeled span P/R/F1、exact-span role accuracy 与逐句错误,不能只凭单句手测;`.superpowers/acceptance/run-core-gold-evaluation.mjs` 仅手动真模型评测,不进 CI。runner base URL 只接受 HTTP(S),拒绝 username/password/query/fragment；网络请求、日志与 artifact 统一使用去尾斜杠的规范化安全 URL。评分前对**预测**成分做尾标点归一化(纯标点成分丢弃 + 句尾终止标点尾巴裁剪,与生产 validator `semanticComponents` 同口径;gold 不动,`fragment-portable-api` 尾标点例外保持可区分)——不归一化会让按 prompt「成分可含标点」行事的模型被 exact/span 记账双重惩罚;归一化揭示的真退化才计入回归(v14 回归靶:系表合并、OBJECT 吞分词后置 ATTRIBUTE、图注双重合并、非限定 when 教学空隙)。页面级验收以页面 corpus 单独三次配对评测,不能与 40 句 corpus 总分混比。
...
- **自动扫描是页面语义清单的投影,显式手势不套用它的取舍**:`scanDocument` = `page-inventory` 枚举的语义单元里 `automatic === true` 的那部分——先 `selectPrincipalRoot` 选正文容器,再按分类型门槛判自动资格(科学语义排除前置;英文占比 ≥ 60%;**统一 20 字符门已取消**,仅 loose div/section/span 与非语义类保留最短 20 字符,标题/dt/th/caption 只要有可读英文实词,p/li/dd/td/figcaption/callout/.ltx_bib_title 靠语义标签 + principal root + 英文占比把关),最后父子去重(li>p / td>p / figcaption>p 只进子,table/tr/figure 永不整体成块)。文本提取由 `readable-dom-text` 统一负责(`<math>` 一律读剔除 annotation/annotation-xml/mphantom/隐藏子树后的**可见 MathML 文本**;`alttext` 是 TeX 源码,弃用;嵌套 math 只读一次)。`nearestSafeBlock` 只服务用户指到的那一处,不走正文容器与最短长度这两道统计门(英文占比仍适用,非英文内容照旧拒绝)——曾经套用后表现为「鼠标明明停在段落上,快捷键却报『未找到可解析的段落』」(多 article 页面、SPA 换内容后缓存失效、短段落全中招)。显式路径**按渲染盒子而非标签名认块**(`isRenderedBlock` 看 computed display,`LOOSE_BLOCK_SELECTOR` 只作兜底)——Mintlify 一类文档站(含 Claude Code 自己的文档)整篇正文都是 `<span data-as="p">`,只按标签名认块会把这类站点整页判成「未找到」。同时**只认叶子块**(`hasBlockChild`),否则往上找会撞到包着整篇正文的外层容器。注意 happy-dom 里内联元素的 computed display 是空串而非 `"inline"`,判据要把空串算作非块。给 `nearestSafeBlock` 加回任何「正文容器/最短长度」限制前,先想清楚它只有显式调用方。IntelliJ 侧同源但范围独立:`scanMarkdownBlocks` 的 Markdown 语义块(短语义标签 h1-h6/dt/caption/th 有英文实词即可、其余候选维持 20 字符)与连字符自定义元素,`nearestPreviewBlock` 四条判据不套 20 字符/英文占比门。

- **repair 错误坐标必须与模型所见 JSON 一致**:validator 按句单独调用、错误路径恒以 `sentences[0]` 起,多句错误必须按 `sentenceId` 重新分组(四类路径 invalid/missing/duplicate + `rawOccurrence`)才能进 repair prompt;组内 `components[k]` 用**原始数组下标**(纯标点成分被跳过但占位),语法邻接仍按语义成分序列判——两套坐标系分离,接受集合与成功成分不得因坐标改动而变。`<math>` 一律读可见 MathML 文本(`alttext` 是 TeX 源码,弃用)。「通过校验但标错」用固定审计集(`audit-silent-mislabel.json`)与失败率分开度量,详见 `docs/architecture/invariants.md`。
