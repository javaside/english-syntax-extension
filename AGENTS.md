...
- **本地可判的语法规则必须进双端 validator**:TS/Kotlin 只同步执行代码实际可判的九条硬门:组件序列相邻且 Token 区间连续的两个 `PREDICATE` 合并;去掉标点后恰好一个 lexical word、role 不是 `CONJUNCTION` 且命中**保守的高把握“必须带宾语”介词白名单**的成分并入管辖短语(常见副词/表语/连词兼类词 `after/before/down/off/over/since/until/around/inside/outside` 不收);`COORDINATE_CLAUSE` 数量恰好为 1 时拒绝;`CONJUNCTION` 的 lexical words 至少含一个 FANBOYS;`PREDICATE` 首个 lexical word 是限定词/主格代词/`that` 时拒绝(动词组不可能以它们开头,命中即主语被吞);`PREDICATE` 非首位 lexical words 含限定词时拒绝(限定词是名词短语左边界,`that` 刻意不在这条里);`COORDINATE_CLAUSE` 首词是从属连词且整句无 `CONJUNCTION` 时拒绝(有 `CONJUNCTION` 就放行——`Because A, B, and C` 的第一个并列分句本来就这样开头);单个成分覆盖全部非标点 token 且句子实词数 ≥ 4 时拒绝(不论 role,那等于没有划分);出现 2 个以上 `COORDINATE_CLAUSE` 却既没有 `CONJUNCTION` 成分也没有 `;` token 时拒绝(并列句 = 各分句自带主语 + 并列连词或分号连接)。**缺主语本身不判**——祈使句本来就没有主语,文档里 `First, install the CLI.` 这类副词开头的祈使句更常见,按缺主语判会大面积误拒;`then` 是副词,不进从属连词表。grammar 判定只由结构可信度把关(每个 component 都有可用 range/role/translation、区间句内、有序不重叠、非纯标点),unknown field、translation too long、sentenceId 等非结构错误不得阻断同轮 grammar 诊断,两类错误必须同次返回。错误英文文案逐字一致并进入 repair prompt。**黄金集必须整份通过这套校验**(`core-gold-annotations.test.ts` 钉住):新硬门把正确答案判非法比漏判更糟,那会把合法分析送进无意义的修复轮。
- **tokenization 变化同时升两条提示词版本**:core span 与 detail focus 都依赖 Token ID,所以分词改动必须同时提升 `CORE_PROMPT_VERSION` 与 `DETAIL_PROMPT_VERSION`;当前为 `CORE_SCHEMA_VERSION=3`、core `7`、detail `5`。
