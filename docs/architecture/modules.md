...
| `indexed-components.ts`         | validator 内部的诊断坐标载体：`rawComponentEntries()` 先记录原始下标再排除纯标点成分（判据与旧过滤逐条等价），`toIndexed()` 只留成功解析项。**rawIndex 只进错误路径，不进返回给渲染与缓存的 components**——否则 repair prompt 的错误会指向模型所见 JSON 的隔壁成分 |
| `analysis-validator.ts`         | `validateCoreBatch()` / `validateDetail()`——模型输出的最终裁判：结构/覆盖率/安全校验，以及十六条本地语法粒度硬约束（连续相邻谓语、单词白名单介词、废弃的 `COORDINATE_CLAUSE`、至少含 FANBOYS 的连词、短语角色吞从属连词、谓语首词是限定词/主格代词、谓语内部含限定词、从属连词引导的并列分句、单成分包住整句并对片段角色设 10 实词上限、单词从句、主语从句引导词闭集、定语从句后紧跟其宾语/表语、名词性成分吞入后置 `of` 短语、成分以必带宾语介词收尾、`FRAGMENT_HEAD` 至多一个、`FRAGMENT_HEAD` 不与分句级角色混用）；错误文案直接供 repair prompt 使用                        |
...
| `language/IndexedComponents.kt`                      | 诊断坐标载体(与 TS `indexed-components.ts` 同构)                                                                                                                                                                                 |
| `language/AnalysisValidator.kt`                       | 模型输出校验(与 TS `analysis-validator.ts` 同规则)                                                                                                                                                                               |
| `analysis/RepairErrors.kt`                             | repair 错误按句分组(与 TS `repair-errors.ts` 同构)                                                                                                                                                                               |                                                                                                                                                                               |
...
| `scripts/silent-mislabel-audit.mjs`        | 静默错标审计器:固定审计集(token 区间约束 oracle + 可选完整 `acceptedAnalyses`)对「通过校验但标错」做两轴(运行×裁决)判定。局部约束通过只报 `meets-constraints`;只有完整答案逐组整体命中才报 `meets-gold` 并进正确分子;缺逐句终态证据报 `not-terminal`;oracle 用生产 tokenizer 校验坐标。CLI 消费真机报告 |
