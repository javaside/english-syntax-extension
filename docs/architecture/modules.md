...
| `analysis-validator.ts`         | `validateCoreBatch()` / `validateDetail()`——模型输出的最终裁判：结构/覆盖率/安全校验，以及八条本地语法粒度硬约束（连续相邻谓语、单词白名单介词、恰好一个并列分句、至少含 FANBOYS 的连词、谓语首词是限定词/主格代词、谓语内部含限定词、从属连词引导的并列分句、单成分包住整句）；错误文案直接供 repair prompt 使用                        |
