...
- 译文质量门的共享 case 由 `shared-fixtures/translation-quality.json` 双端 replay：拒绝原样回显、大小写/空白差异回显、全宽字符回显、无 Han 非回显、纯专名（JSON / Spring AI / GWTC-5.0 / H0）；接受「丢名 + 中文类型」释义（JSON 数据格式、Spring AI 框架、GWTC-5.0 引力波事件目录、哈勃常数 H0）与含 translation+grammar 同轮完整错误序列的 case。双端各自用生产 `tokenize()` 重建 Token 后断言 `accepted` 与完整有序 errors。fixture 另有 `hanScriptBoundary` 组（15 case：假名/谚文/CJK 标点/纯 ASCII 期望 false，扩展 A 区 `㐀`、CJK 兼容区 `⾀`、`〇` 期望 true），双端各自把生产 Han 判定（TS `\p{Script=Han}` / JVM `\p{IsHan}`）对着它全量 replay，钉住跨区块脚本等价。
