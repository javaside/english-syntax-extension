...
页面级 prompt/validator 收口(`CORE_PROMPT_VERSION` 15 / `DETAIL_PROMPT_VERSION` 8,`CORE_SCHEMA_VERSION` 3 未变)以**两套 corpus、各三次 baseline/candidate 配对**验收,`--compare-manifest` 机器校验 corpus/sentenceOrder/endpoint/model/batch/temperature/reasoning/response-format/timeout 逐项一致:
