...
本节说明生产实现的校验/修复机制；十五类硬门逐项解释、不能硬判的语言学边界和新增规则的决策方法见 [`sentence-component-analysis.md` §7–9](./sentence-component-analysis.md#7-本地-validator-的能力边界)。
...
- `validateCoreBatch` 还把十五条**代码实际可判的语法粒度约束**变成硬门：组件序列相邻且 Token 区间连续的两个 `PREDICATE` 必须合并；成分去掉标点后恰好一个 lexical word、role 不是 `CONJUNCTION` 且命中保守的高把握“必须带宾语”介词白名单时，必须并入其管辖短语（`after/before/down/off/over/since/until/throughout/around/inside/outside` 等常见副词、表语或连词兼类词不收）；`COORDINATE_CLAUSE` 出现即非法（≥1 个，该角色已废弃）；`CONJUNCTION` 至少含一个 FANBOYS；`ADVERBIAL` / `ATTRIBUTE` 以保守从属连词门开头时非法（5 词无条件表 `although/whereas/unless/lest/whilst`，另对 `because` 排除 `because of`、对 `though` 放行句尾单词）；`PREDICATE` 首个 lexical word 是限定词、主格人称代词或 `that` 时非法；`PREDICATE` 非首位 lexical words 含限定词时非法（`that` 刻意不算，它更常是宾语从句引导词）；单个成分覆盖全部非标点 token 且句子实词数 ≥ 4 时通常非法；片段语义角色 `{FRAGMENT_HEAD, INDEPENDENT_ELEMENT, APPOSITIVE}` 在 `MAX_WHOLE_SENTENCE_FRAGMENT_LEXICAL_TOKENS = 10` 以内豁免，超过 10 仍要求拆成片段主体与修饰语（10 是挑战集审阅后的启发式上限，不是语法定律）；`COORDINATE_CLAUSE` 首词是从属连词且整句无 `CONJUNCTION` 成分时另有补充错误（旧的「≥2 个并列分句须有连词或分号」判据已被废弃门整体覆盖）；五类从句角色的成分只有 1 个 lexical word 时非法（从句至少是引导词 + 谓语，或主语 + 谓语）；`SUBJECT_CLAUSE_INTRODUCERS = 15` 的闭集额外约束主语从句首词（刻意不收 `if/however`）；`ATTRIBUTIVE_CLAUSE` 的下一个成分是 `OBJECT` / `PREDICATIVE` 时非法（紧跟的宾语/表语一定是从句自己的）；`COMPLEMENT` 刻意豁免，宾补结构 `consider the movie that she directed a masterpiece` 合法；非从句角色成分的最后一个 lexical word 命中「几乎不可能在短语内悬垂」的介词表（`of/into/onto/upon/within/among/between/despite/during/toward/towards`）时非法；五类从句角色豁免（`what dreams are made of` 中介词可在从句内部合法悬垂），短语角色 `near the frontier of` 仍非法。**最后两条是第 17 个角色 `FRAGMENT_HEAD` 的边界**：至多一个；存在时整句不得混入 `FRAGMENT_FORBIDDEN_ROLES` 里列举的分句级角色（`SUBJECT` / `PREDICATE` / `OBJECT` / `PREDICATIVE` / `COMPLEMENT`、四类从句角色 `SUBJECT_CLAUSE` / `OBJECT_CLAUSE` / `PREDICATIVE_CLAUSE` / `ADVERBIAL_CLAUSE`、已废弃的 `COORDINATE_CLAUSE`）——唯一例外是完整定语从句 `ATTRIBUTIVE_CLAUSE` 可以紧跟片段主体（fragment-relative：`An API` + `that returns JSON responses`），放行集合用显式集合枚举、不再展开 `CLAUSE_ROLES`；十五条硬门总数不变，其余从句门（最小长度、follower 门、尾介词豁免、主语从句首词闭集）照旧约束放行的从句。其余混标等于给不成句的输入虚构主谓宾，错误文案分别为 `"a non-clausal fragment must contain at most one FRAGMENT_HEAD"` 与 `"FRAGMENT_HEAD marks a non-clausal fragment and must not be mixed with clause-level SUBJECT, PREDICATE, OBJECT, PREDICATIVE, COMPLEMENT, SUBJECT_CLAUSE, OBJECT_CLAUSE, PREDICATIVE_CLAUSE, ADVERBIAL_CLAUSE, or the deprecated COORDINATE_CLAUSE; a whole ATTRIBUTIVE_CLAUSE is the only clause role allowed beside a FRAGMENT_HEAD"`（文案显式列出仍禁止的角色，不再说 all clause roles）。**validator 刻意不用词表判定「缺限定谓语」**：词形兼类（祈使句、动名词短语）会大面积误拒，模型该用而没用 `FRAGMENT_HEAD` 由提示词、黄金集与真模型评测约束。后四条补的正是「模型给出的坏划分照样显示给用户」的那一段：实测 deepseek-chat 把 `She kept practicing until…` 整句只标成 `PREDICATE` + `ADVERBIAL_CLAUSE`（一个主语都没有），前四条一条都拦不住。最后三条从句硬门补的是另一段：实测把 `that` 单独标成 `ATTRIBUTIVE_CLAUSE`、从句的谓语与宾语平铺到主句层，页面上出现两个同级"谓语"，而这个划分**完全通过**当时的全部硬门。**缺主语本身刻意不判**——祈使句没有主语，`First, install the CLI.` 这类副词开头的祈使句在文档里很常见，按缺主语判会大面积误拒；改判「谓语开头不可能是动词」既严密又覆盖这个失败。**介词悬垂表刻意不收 `for`/`with`/`at`/`from`/`to`**——关系从句里 `the tool I work with`、`the place I came from` 让它们合法地出现在成分末尾。grammar 只在结构可信时执行：所有 component 都有可用 range/role/translation、区间句内、有序不重叠、非纯标点；unknown field、translation too long、sentenceId 等非结构错误不阻止同轮 grammar 诊断，两类错误同次报告。TS/Kotlin 逐条、逐文案一致，错误会原样进入 repair prompt。黄金集整份必须通过这套校验（`core-gold-annotations.test.ts`）。
...
### 8.1 成分粒度的边界与并列句平铺(`CORE_PROMPT_VERSION` 8 起禁用 `COORDINATE_CLAUSE`；当前契约为 16)
